...
class ArgsManager:
    ...
    def parseArgs(self, parse: bool = True):
        """
        ...
        parser.add_argument("-lrct", "--lrcThreshold", default=4, type=self._checkRange(0, 10),
                            help="Left right check threshold, used for depth measurement. Default: %(default)s")
        parser.add_argument("-sig", "--sigma", default=0, type=self._checkRange(0, 250),
        ...
        parser.add_argument('-scale', '--scale', type=self._comaSeparated(default=0.5, cast=float), nargs="+",
        ...
        parser.add_argument("-cm", "--colorMap", default="JET", choices=self._colorMaps, help="Change color map used to apply colors to depth/disparity frames. Default: %(default)s")
        ...
        parser.add_argument('-s', '--show', default=[], nargs="+", choices=self._streamChoices, help="Choose which previews to show. Default: %(default)s")
        ...
        parser.add_argument('-cb', '--callback', type=Path, default=self.folderPath / 'callbacks.py', help="Path to callbacks file to be used. Default: %(default)s")
        parser.add_argument("--openvinoVersion", type=str, choices=self._openvinoVersions, help="Specify which OpenVINO version to use in the pipeline")
        ...
        parser.add_argument('-enc', '--encode', type=self._comaSeparated(default=30.0, cast=float), nargs="+", default=[],
        ...
        parser.add_argument("--cameraContrast", type=self._comaSeparated("all", int), nargs="+", help="Specify image contrast")
        parser.add_argument("--cameraBrightness", type=self._comaSeparated("all", int), nargs="+", help="Specify image brightness")
        parser.add_argument("--cameraSharpness", type=self._comaSeparated("all", int), nargs="+", help="Specify image sharpness")
        parser.add_argument("--irDotBrightness", type=self._checkRange(0, 1200), default=0, help="For OAK-D Pro: specify IR dot projector brightness, range: 0..1200 [mA], default 0 (turned off)")
        parser.add_argument("--irFloodBrightness", type=self._checkRange(0, 1500), default=0, help="For OAK-D Pro: specify IR flood illumination brightness, range: 0..1500 [mA], default 0 (turned off)")
    ...
    def _checkRange(self, minVal, maxVal):
        def checkFn(value):
            ivalue = int(value)
    ...
    def _comaSeparated(self, default, cast=str):
        def _fun(option):
    ...
    def _orientationCast(self, arg):
        if not hasattr(dai.CameraImageOrientation, arg):
            raise argparse.ArgumentTypeError("Invalid camera orientation specified: '{}'. Available: {}".format(arg, self._orientationChoices))
