...
class NodeGraph(QtCore.QObject):
    ...
    def _build_context_menu(self):
        ...
        """
        from ..base.graph_actions import build_context_menu
    ...
    def _on_property_bin_changed(self, node_id, prop_name, prop_value):
        ...
        # GX: fix for: changing the name in property_bin does not resize
    ...
    def lock_all_ports(self):
        for n in self.all_nodes():
            if isinstance(n, BaseNode):
                for p in n.input_ports():
                    if not p.locked():
                        p.set_locked(True, push_undo=False)
                for p in n.output_ports():
                    if not p.locked():
                        p.set_locked(True, push_undo=False)

    def unlock_all_ports(self):
        for n in self.all_nodes():
            if isinstance(n, BaseNode):
                for p in n.input_ports():
                    if p.locked():
                        p.set_locked(False, push_undo=False)
                for p in n.output_ports():
                    if p.locked():
                        p.set_locked(False, push_undo=False)
    ...
    def clear_session(self):
        ...
        for n in self.all_nodes():
            if isinstance(n, BaseNode):
                for p in n.input_ports():
                    if p.locked():
                        # GX If connected_ports=False, 
                        # NodeGraphQt.errors.PortError: Can't disconnect port because "[5] in" is locked.
                        # p.set_locked(False, connected_ports=False)
                        p.set_locked(False, connected_ports=True)
                    p.clear_connections()
                for p in n.output_ports():
                    if p.locked():
                        # p.set_locked(False, connected_ports=False)
    ...
    def save_session(self, file_path):
        ...
        # GX Without 2 lines below, the file path is not memorized
    ...
    def import_session(self, file_path):
        ...
        self.session_changed.emit(file_path)
        # GX Without line below, when load a graph, the ports are unlocked
        self.lock_all_ports()
