...
def decode_openpose(nnet_packet, **kwargs):
    """
    Decode opencv using the opencv using opencv.

    Args:
        nnet_packet: (todo): write your description
    """
    # openpose from openvino has pafs in first output, keypoints in second
    output_list = nnet_packet.getOutputsList()
    heatmaps = np.copy(output_list[1])
    pafs = np.copy(output_list[0])
