...
def start_ir_handler(dev):
    if not dev.is_projector_connected():
        print("IR projector not detected! Exiting the IR handler task...")
        return
    class IrLogWrapper:
        ...
        def irWriteReg(self, reg, value):
            ...
            device_id = 0 # TODO for multiple projectors
            self.dev.send_ir_write_command(device_id, reg, value)
            self.idx += 1
        def irReadReg(self, reg):
            value = 0 # TODO self.dev.irReadReg(reg)
