...
def parse_args():
    ...
    if options.markerSizeCm is None:
        if options.defaultBoard:
            ...
            raise argparse.ArgumentError(options.markerSizeCm,
                                         "-ms / --markerSizeCm needs to be provided (you can use -db / --defaultBoard if using calibration board from this repository or calib.io to calculate -ms automatically)")
...
class Main:
    ...
    def show_failed_capture_frame(self):
        ...
        # self.show_img("left", info_frame)
        # self.show_img("right", info_frame)
        self.show_img(self.display_name, info_frame)
        self.waitKey(2000)
    ...
    def capture_images_sync(self):
        ...
        while not finished:
            ...
            for name, imgFrame in currImageList.items():
                ...
                if (widthRatio > 0.8 and heightRatio > 0.8 and widthRatio <= 1.0 and heightRatio <= 1.0) or (
                        widthRatio > 1.2 and heightRatio > 1.2) or (resizeHeight == 0):
                    resizeWidth = width
                    resizeHeight = height
            ...
            for name, imgFrame in currImageList.items():
                ...
                subImage = np.pad(imgFrame, ((height_offset, height_offset), (width_offset, width_offset)), 'constant',
                                  constant_values=0)
            ...
            self.show_img(self.display_name, combinedImage)
            tried = {}
            allPassed = True
            ...
            if capturing:
                ...
                if allPassed:
                    if not self.images_captured:
                        leftStereo = self.board_config['cameras'][self.board_config['stereo_config']['left_cam']][
                            'name']
                        rightStereo = self.board_config['cameras'][self.board_config['stereo_config']['right_cam']][
                            'name']
    ...
    def capture_images(self):
        ...
        while not finished:
            ...
            for packet in recent_frames:
                ...
                lrgb_time = 0
                if not self.args.disableRgb:
                    lrgb_time = min([abs((recent_left.getTimestamp() - recent_color.getTimestamp()).microseconds), abs((
                                                                                                                                   recent_color.getTimestamp() - recent_left.getTimestamp()).microseconds)]) / 1000
                lr_time = min([abs((recent_left.getTimestamp() - recent_right.getTimestamp()).microseconds),
                               abs((recent_right.getTimestamp() - recent_left.getTimestamp()).microseconds)]) / 1000
                ...
                # self.show_img(packet.stream_name, small_frame)
            ...
            if start_timer == True:
                ...
                cv2.putText(combine_img, str(timer),
                            ...
                            7, (0, 255, 255),
                            4, cv2.LINE_AA)
            self.show_img(self.display_name, combine_img)
            frame_list.clear()
    ...
    def calibrate(self):
        ...
        try:
            ...
            for camera in result_config['cameras'].keys():
                ...
                calibration_handler.setCameraIntrinsics(stringToCam[camera], cam_info['intrinsics'],
                                                        cam_info['size'][0], cam_info['size'][1])
                ...
                if 'extrinsics' in cam_info:
                    ...
                    if 'to_cam' in cam_info['extrinsics']:
                        ...
                        # pygame_render_text(self.screen, text, (vis_x, vis_y), color, 30)
                        # vis_y += 30
                        specTranslation = np.array([cam_info['extrinsics']['specTranslation']['x'],
                        ...
                        elif result_config['stereo_config']['left_cam'] == cam_info['extrinsics']['to_cam'] and \
                                result_config['stereo_config']['right_cam'] == camera:
                            calibration_handler.setStereoRight(stringToCam[camera],
                                                               result_config['stereo_config']['rectification_right'])
                            calibration_handler.setStereoLeft(stringToCam[cam_info['extrinsics']['to_cam']],
                                                              result_config['stereo_config']['rectification_left'])
