import React, {useEffect, useState} from 'react'
import {Button, Select, Spin} from "antd";
import {PlayCircleOutlined} from "@ant-design/icons";
import {useDispatch, useSelector} from "react-redux";
import {sendConfig, updateAIConfig, updatePreview, updatePreviewConfig} from "../store";

const emptyImg = "data:image/png;base64,"

const CameraPreview = () => {
  const fetched = useSelector((state) => state.demo.fetched)
  const preview = useSelector((state) => state.demo.config).preview || {}
  const devices = useSelector((state) => state.demo.config).devices || {}
  const dispatch = useDispatch()

  return (
    <div className="preview-container">
      {
        !fetched && <Spin tip="Loading..."/>
      }
      <img alt="camera-gui" className="stream-preview " src={fetched ? "/stream" : emptyImg}/>
      <div className="preview-controls">
        <Select value={preview.current} onChange={current => {
          dispatch(updatePreviewConfig({current}))
          dispatch(updatePreview())
        }}>
          {
            (preview.available || []).map(value => (
              <Select.Option value={value}>{value}</Select.Option>
            ))
          }
        </Select>
        <Select value={devices.current} onChange={current => update({current})}>
          <Select.Option value={devices.current}>{devices.current}</Select.Option>
          {
            (devices.available || []).map(value => (
              <Select.Option value={value}>{value}</Select.Option>
            ))
          }
        </Select>
        <Button danger onClick={() => dispatch(sendConfig())} type="primary" shape="round" icon={<PlayCircleOutlined/>} size="large">
          Reload
        </Button>
      </div>
    </div>
  );
};

export default CameraPreview;