...
class Main:
                ...
                for in_cam in self.board_config['cameras'].keys():
                    ...
                    if cam_info["name"] not in self.args.disableCamera:
                        if properties.socket == stringToCam[in_cam]:
                            ...
                            if self.args.calib_per_ccm is not {}:
                                if cam_info["name"] in self.args.calib_per_ccm.keys():
                                    self.board_config["cameras"][in_cam]["calib_model"] = self.args.calib_per_ccm[cam_info["name"]]
    ...
    def draw_corners(self, frame, displayframe, color, name):
        marker_corners, ids, charuco_corners, charuco_ids = self.detect_markers_corners(frame)
        for corner in charuco_corners:
            corner_int = (int(corner[0][0]), int(corner[0][1]))
            cv2.circle(displayframe, corner_int, 8*displayframe.shape[1]//1900, color, -1)
        if self.args.useCharucos:
            if name not in self.charucos.keys():
                self.charucos[name] = []
            self.charucos[name].append([charuco_ids, charuco_corners])
            ...
            if capturing:
                ...
                if allPassed:
                    ...
                    for name, frameMsg in syncedMsgs.items():
                        ...
                        if len(self.coverageImages[name].shape) != 3:
                            self.coverageImages[name] = cv2.cvtColor(self.coverageImages[name], cv2.COLOR_GRAY2RGB)
                        self.coverageImages[name] = self.draw_corners(frameMsg_frame, self.coverageImages[name], color, name)
    ...
    def calibrate(self):
        ...
        try:
            ...
            status, result_config = self.stereo_calib.calibrate(
                                        ...
                                        charucos=self.charucos
                                        ) # Turn off enable disp rectify
